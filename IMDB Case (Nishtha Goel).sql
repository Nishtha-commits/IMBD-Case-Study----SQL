...
Part 1:
select year, Count(id) as number_of_movies from imdb.movie
group by year
order by year;
Part2:
select month(date_published) as month_num, Count(id) as number_of_movies from imdb.movie
group by month_num
order by month_num;
...
-- Type your code below:
select count(id) as number_of_movies, year from imdb.movie
...
select distinct(genre) as Movie_genre from imdb.genre;
...
-- Type your code below:
select Count(movie_id) as count,genre from imdb.genre
group by genre
...
SELECT * FROM cte_avg_rating_rank WHERE movie_rank<=10; 
...
FROM imdb.genre g
INNER JOIN imdb.movie m
ON m.id = g.movie_id
INNER JOIN imdb.ratings r
ON m.id = r.movie_id
